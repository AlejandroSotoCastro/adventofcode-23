...
// const input = "./day10/testInput5.txt";
...
fs.readFile(input, "utf8", (_err, data) => {
    ...
    while (flag) {
        ...
        measuringGrid[nextPoint.y][nextPoint.x] = nextPoint.symbol;
    ...
    // picks theorem
    const inPoints = area - points.length / 2 + 1;
    console.log(inPoints);
    // console.log(measuringGrid.join("\n"));
    ...
    function fillMeasureGrid() {
        for (let i = 0; i < grid.length; i++) {
            measuringGrid.push([]);
            for (let j = 0; j < grid[i].length; j++) {
                measuringGrid[i].push(".");
            }
        }
    }
