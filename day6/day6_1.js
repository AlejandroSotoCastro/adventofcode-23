...
fs.readFile(input, "utf8", (err, data) => {
    ...
    const time = rawTime.split(": ")[1].trim().split(/\s+/).map(Number);
    const distance = rawDistance.split(": ")[1].trim().split(/\s+/).map(Number);

    // each race has a distance and a time Example: {distance: 1000, time: 10}
    const races = time.map((t, i) => ({ distance: distance[i], time: t }));

    const allPossibleWays = [];
    races.forEach((race) => {
        allPossibleWays.push(calculateRace(race));
    });
